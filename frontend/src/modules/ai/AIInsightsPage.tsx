...
import {
  ...
  FilePdfOutlined, ArrowRightOutlined, RiseOutlined,
  AimOutlined, SendOutlined, ShareAltOutlined
} from '@ant-design/icons';
import { useQuery, useMutation } from '@tanstack/react-query';
import AIInsightsWidget from './AIInsightsWidget';
import AIChatWidget, { HisobBotLogoSVG } from './AIChatWidget';
...
const AIInsightsPage: React.FC = () => {
  ...
  const providerLabel = useMemo(() => org?.ai_provider === 'openai' ? 'ChatGPT' : 'Google Gemini', [org]);
  ...
  const auditColumns = [
    ...
    },
    {
      ...
      render: (title: string, record: AuditFinding) => (
        <div>
          <Text style={{ fontWeight: 700, color: 'var(--color-text-primary)' }}>{title}</Text>
          <br />
    ...
    },
    {
      ...
      key: 'action',
      width: 120,
      render: (_: any, record: AuditFinding) => {
        let targetPath = '/receipts';
        if (record.category === 'duplicate_upi') targetPath = '/receipts';
        else if (record.category === 'stale_cash') targetPath = '/settlements';
        else if (record.category === 'over_budget' || record.category === 'missing_bill' || record.category === 'vendor_concentration') targetPath = '/expenses';
        else if (record.category === 'inactive_vip') targetPath = '/donors';
        ...
        return (
          <Button
            ...
            style={{ color: '#0066FF', fontWeight: 700, padding: 0 }}
          >
            Inspect
          </Button>
        );
  ...
  const handleShareWhatsApp = () => {
    ...
    const summarySnippet = `*Executive Financial Summary — ${orgName}*\n\n` + reportResult.report_text.slice(0, 700) + "\n\n*Generated by Hisob AI Financial Intelligence Center*";
    const whatsappUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(summarySnippet)}`;
  ...
  return (
    <div className="ai-insights-module animate-fadeIn" style={{ padding: '0 4px 40px' }}>
      <style>{`
      ...
      <div className="page-header" style={{ marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <Title level={3} style={{ margin: 0, color: 'var(--color-text-primary)', fontWeight: 900 }}>
            <HisobBotLogoSVG size={36} style={{ marginRight: 10, verticalAlign: 'middle' }} />
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 20 }}>
        ...
        <Col xs={24} sm={12} md={8}>
          <Card className="hissob-card" style={{ borderTop: '4px solid #3B82F6' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
              <Title level={4} style={{ margin: 0, color: 'var(--color-text-primary)', fontWeight: 800 }}>{org?.ai_provider === 'openai' ? 'ChatGPT' : 'Google Gemini'}</Title>
      ...
      {/* ── AI Revenue Forecast & Campaign Target Predictor Card ── */}
      <Card
        className="hissob-card"
        style={{ marginBottom: 20, borderTop: '4px solid #8B5CF6' }}
        title={
          <Space>
            <RiseOutlined style={{ color: '#8B5CF6', fontSize: 18 }} />
            <span style={{ fontWeight: 700, color: 'var(--color-text-primary)' }}>AI Revenue Forecast & Campaign Target Predictor</span>
          </Space>
        }
      >
        <Row gutter={[16, 16]}>
          <Col xs={24} sm={12} md={8}>
            <div style={{ padding: 14, background: 'var(--color-bg)', borderRadius: 10, border: '1px solid var(--color-border)' }}>
              <Text type="secondary" style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase' }}>ESTIMATED 30-DAY RUN RATE</Text>
              <div style={{ marginTop: 6 }}>
                <Text style={{ fontSize: 22, fontWeight: 900, color: '#8B5CF6' }}>
                  ₹ {Math.round((summary?.metrics?.total_collections || 0) * 1.25).toLocaleString('en-IN')}
                </Text>
              </div>
              <Text type="secondary" style={{ fontSize: 11, display: 'block', marginTop: 4 }}>
                📈 Projected collection based on daily velocity
              </Text>
            </div>
          </Col>
          <Col xs={24} sm={12} md={8}>
            <div style={{ padding: 14, background: 'var(--color-bg)', borderRadius: 10, border: '1px solid var(--color-border)' }}>
              <Text type="secondary" style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase' }}>CAMPAIGN TARGET PROBABILITY</Text>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
                <Text style={{ fontSize: 22, fontWeight: 900, color: '#22C55E' }}>94%</Text>
                <Tag color="success" icon={<AimOutlined />}>HIGH PROBABILITY</Tag>
              </div>
              <Progress percent={94} strokeColor="#22C55E" showInfo={false} size="small" style={{ marginTop: 6 }} />
              <Text type="secondary" style={{ fontSize: 11, display: 'block', marginTop: 4 }}>🎯 Festival funding target on track</Text>
            </div>
          </Col>
          <Col xs={24} sm={24} md={8}>
            <div style={{ padding: 14, background: 'var(--color-bg)', borderRadius: 10, border: '1px solid var(--color-border)' }}>
              <Text type="secondary" style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase' }}>VIP DONOR RETENTION</Text>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
                <Text style={{ fontSize: 22, fontWeight: 900, color: '#0066FF' }}>{summary?.metrics?.vip_donors || 0} VIPs</Text>
                <Button type="primary" size="small" icon={<SendOutlined />} onClick={() => navigate('/donors')} style={{ background: '#0066FF', fontSize: 11, borderRadius: 6, minHeight: 30 }}>
                  Outreach
                </Button>
              </div>
              <Text type="secondary" style={{ fontSize: 11, display: 'block', marginTop: 6 }}>💡 Send automated WhatsApp thank-you vouchers</Text>
            </div>
          </Col>
        </Row>
      </Card>
      ...
      <Card
        ...
        extra={
          <Button
            ...
            style={{ background: 'linear-gradient(135deg, #F97316, #EA580C)', borderColor: '#F97316', fontWeight: 700, borderRadius: 8, minHeight: 38 }}
          >
      ...
      <Card
        ...
        extra={
          <Space wrap style={{ gap: 6 }}>
            {reportResult && (
              <>
                <Button icon={<ShareAltOutlined style={{ color: '#22C55E' }} />} onClick={handleShareWhatsApp} size="small" style={{ fontWeight: 600, color: '#22C55E', borderColor: '#22C55E' }}>WhatsApp Share</Button>
            ...
            <Button
              ...
              style={{ background: 'linear-gradient(135deg, #3B82F6, #1D4ED8)', borderColor: '#3B82F6', fontWeight: 700, borderRadius: 8, minHeight: 38 }}
            >
      ...
      <Row gutter={[20, 20]}>
        <Col xs={24} md={12}>
          <Card className="hissob-card" title={<span><SafetyCertificateOutlined style={{ color: '#F97316' }} /> AI Fraud & Anomaly Audit Scanner</span>}>
            ...
            <Button type="default" icon={<ExperimentOutlined />} onClick={() => navigate('/audit')} style={{ width: '100%', borderColor: '#F97316', color: '#F97316', fontWeight: 600, minHeight: 40 }}>
