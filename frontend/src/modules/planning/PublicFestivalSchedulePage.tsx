...
export const PublicFestivalSchedulePage: React.FC = () => {
  ...
  if (error || !data) {
    return (
      <div style={{ maxWidth: 600, margin: '60px auto', textAlign: 'center', padding: 24 }}>
        <Title level={3} style={{ color: '#0B2347' }}>Festival Schedule Not Found</Title>
        <Text type="secondary">The public event schedule link might be invalid or the festival has concluded.</Text>
      </div>
    );
  }
  ...
  // Sort: Today & Upcoming events FIRST at the top, Completed (past) events LAST at the bottom.
  // Multiple events on the SAME DAY are sorted chronologically by time (Morning AM -> Afternoon -> Evening PM)
  ...
  return (
    <div style={{ backgroundColor: '#F1F5F9', minHeight: '100vh', paddingBottom: 60, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ maxWidth: 840, margin: '-28px auto 0 auto', padding: '0 16px', position: 'relative', zIndex: 10 }}>
        ...
        {/* ── Section 1: "Happening Today" 🔴 LIVE Section ── */}
        <Card
          style={{
            borderRadius: 18,
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <Title level={4} style={{ margin: 0, color: '#0B2347', display: 'flex', alignItems: 'center', gap: 8, fontWeight: 800 }}>
          ...
          {todayEvents.length > 0 ? (
              ...
              {todayEvents.map((ev: any) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Text strong style={{ fontSize: 16, color: '#0B2347' }}>
                  ...
                  <Text type="secondary" style={{ fontSize: 13 }}>
                    <ClockCircleOutlined /> Time: <strong>{ev.start_time || 'All Day'}</strong> • <EnvironmentOutlined /> {ev.location}
                  </Text>
                  ...
                  {ev.description && (
                    <Text type="secondary" style={{ fontSize: 12 }}>{ev.description}</Text>
                  )}
            ...
            <div style={{ textAlign: 'center', padding: '16px 0' }}>
              <Text type="secondary">No scheduled programs for today. Check the full festival schedule below!</Text>
            </div>
          )}
        </Card>

        {/* ── Section 2: Full Festival Timetable & Filter ── */}
        <Card style={{ borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }} styles={{ body: { padding: '24px' } }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
            <Title level={4} style={{ margin: 0, color: '#0B2347' }}>Full Event Schedule & Timetable</Title>

            {/* Event Category & History Filter Buttons */}
            <Space wrap size={[6, 6]}>
              <Button
                size="small"
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {sortedSchedules.length > 0 ? (
              ...
              return (
                <div
                  ...
                  style={{
                    padding: '16px',
                    borderRadius: 12,
                    border: isPast ? '1px solid #E2E8F0' : isToday ? '1px solid #FDBA74' : '1px solid #E2E8F0',
                    borderLeft: isPast ? '4px solid #94A3B8' : isToday ? '4px solid #F97316' : '4px solid #3B82F6',
                    backgroundColor: isPast ? '#F8FAFC' : isToday ? '#FFFBF5' : '#FFFFFF',
                    display: 'flex',
                    flexDirection: 'column',
                    gap: 8,
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <Text strong style={{ fontSize: 16, color: isPast ? '#64748B' : '#0B2347' }}>
                          {tagInfo.icon} {item.title}
                        </Text>
                        {isToday && <Tag color="red" style={{ fontWeight: 800, borderRadius: 8 }}>TODAY 🔴</Tag>}
                        {isPast && <Tag style={{ backgroundColor: '#E2E8F0', color: '#475569', borderRadius: 8, fontSize: 10, fontWeight: 700, border: 'none' }}>✓ COMPLETED</Tag>}
                      </div>
                  ...
                  {item.yajman_name ? (
                    <div style={{
                      ...
                      padding: '6px 10px',
                      borderRadius: 8,
                      border: isPast ? '1px solid #E2E8F0' : '1px solid #FDE68A',
                      display: 'flex',
                      alignItems: 'center',
                    ...
                    <div style={{ backgroundColor: '#F1F5F9', padding: '6px 10px', borderRadius: 8, border: '1px solid #E2E8F0' }}>
                      <Text type="secondary" style={{ fontSize: 12 }}>
                        ✓ Event Concluded
                      </Text>
                    </div>
                  ) : (
                    <div style={{ backgroundColor: '#F8FAFC', padding: '6px 10px', borderRadius: 8, border: '1px dashed #CBD5E1' }}>
                    ...
                    <Space>
                      ...
                      {item.yajman_name ? (
                        <Button
                          ...
                          disabled
                          style={{
                            backgroundColor: isPast ? '#F1F5F9' : '#ECFDF5',
                            borderColor: isPast ? '#E2E8F0' : '#A7F3D0',
                            color: isPast ? '#64748B' : '#059669',
                            fontWeight: 600
                          }}
                        >
                        ...
                        <Button size="small" disabled style={{ backgroundColor: '#F1F5F9', borderColor: '#E2E8F0', color: '#94A3B8' }}>
                          Closed
                        </Button>
                      ) : (
                        <Button
              ...
              <div style={{ textAlign: 'center', padding: '24px 0' }}>
                <Text type="secondary">No programs match your search term "{searchTerm}".</Text>
              </div>
            )}
          </div>
        </Card>
        ...
        {/* Footer */}
        <div style={{ textAlign: 'center', marginTop: 30, color: '#64748B', fontSize: 12 }}>
          <div>Powered by <strong style={{ color: '#0B2347' }}>Hisob ERP</strong> • Festival & Trust Management Platform</div>
      ...
      {/* ── Modal: Sponsor Aarti / Yajman Request ── */}
      <Modal
