...
const PlanningPage: React.FC = () => {
  ...
  const [taskViewMode, setTaskViewMode] = useState<'table' | 'grid'>('grid');
  ...
  // Helper functions to open create & edit modals with populated form fields
  ...
  const handleOpenEditTask = (task: FestivalTask) => {
    setEditingTask(task);
    setIsTaskModalOpen(true);
    setTimeout(() => {
    ...
    if (isTaskModalOpen) {
      if (editingTask) {
        taskForm.setFieldsValue({
          ...
          assigned_to_user_id: editingTask.assigned_to_user_id || undefined,
  ...
  const updateStatusMut = useMutation({
    mutationFn: ({ id, status }: { id: string; status: 'todo' | 'in_progress' | 'completed' | 'cancelled' }) =>
      updateFestivalTask(id, { status }),
    onSuccess: () => {
  ...
  return (
    <div style={{ padding: isMobile ? '12px 8px' : '24px', maxWidth: 1400, margin: '0 auto' }}>
                ...
                <div>
                  <div style={{
                    ...
                    <Space wrap style={{ width: isMobile ? '100%' : 'auto' }}>
                      <Segmented
                        value={taskViewMode}
                        onChange={(val) => setTaskViewMode(val as 'table' | 'grid')}
                        options={[
                        ...
                        return (
                          <Col xs={24} sm={12} md={8} key={t.id}>
                            <Card
                              ...
                              style={{
                                borderRadius: 12,
                                border: isCompleted ? '1px solid var(--color-border)' : '1px solid rgba(249, 115, 22, 0.35)',
                                borderLeft: isCompleted ? '4px solid #10B981' : t.status === 'in_progress' ? '4px solid #3B82F6' : '4px solid #F59E0B',
                            ...
                            >
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 6 }}>
                                <div>
                                  <Text strong style={{ fontSize: 15, color: 'var(--color-text-primary)', textDecoration: isCompleted ? 'line-through' : 'none' }}>
                                    {t.title}
                                  </Text>
                                  {t.description && (
                                    <Paragraph type="secondary" style={{ fontSize: 12, margin: '2px 0 0 0', color: 'var(--color-text-secondary, #94A3B8)' }} ellipsis={{ rows: 2 }}>
                                      {t.description}
                                    </Paragraph>
                                  )}
                                </div>
                                <Tag color={PRIORITY_COLOR[t.priority] || 'blue'} style={{ fontWeight: 700, borderRadius: 6, margin: 0 }}>
                                  {t.priority?.toUpperCase()}
                                </Tag>
                              </div>

                              <div style={{ backgroundColor: 'var(--color-bg-subtle, rgba(255,255,255,0.04))', padding: '6px 10px', borderRadius: 8, margin: '8px 0', border: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <span style={{ fontSize: 12, color: 'var(--color-text-primary)', fontWeight: 600 }}>
                                  👤 {t.assigned_to_name || 'Unassigned'}
                                </span>
                                <Tag color="volcano" style={{ fontSize: 10, margin: 0, borderRadius: 6 }}>
                                  {t.category || 'General'}
                                </Tag>
                              </div>
                              ...
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto', paddingTop: 8, borderTop: '1px solid var(--color-border)' }}>
                                <Select
                                  ...
                                  size="small"
                                  style={{ width: 135 }}
                                  onChange={(newStatus) => updateStatusMut.mutate({ id: t.id, status: newStatus })}
                                >
                                ...
                                <Space size="small">
                                  ...
                                  />
                                  <Popconfirm title="Delete task?" onConfirm={() => deleteTaskMutation.mutate(t.id)}>
                        ...
                        },
                        {
                          ...
                          render: (status, record: FestivalTask) => (
                            <Select
                              ...
                              size="small"
                              style={{ width: 140 }}
                              onChange={(newStatus) => updateStatusMut.mutate({ id: record.id, status: newStatus })}
                            >
                              <Option value="todo">TODO</Option>
                              <Option value="in_progress">IN PROGRESS</Option>
                              <Option value="completed">COMPLETED</Option>
                        ...
                        return (
                          <Col xs={24} sm={12} md={8} key={item.id}>
                            <Card
                              ...
                              style={{
                                borderRadius: 12,
                                border: isPast ? '1px solid var(--color-border)' : '1px solid rgba(249, 115, 22, 0.35)',
                                borderLeft: isPast ? '4px solid #64748B' : '4px solid #F97316',
                                backgroundColor: 'var(--color-bg-container)',
                            ...
                            >
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 6 }}>
                                <div>
                                  ...
                                  <div style={{ fontSize: 12, color: 'var(--color-text-secondary, #94A3B8)', marginTop: 2 }}>
                              ...
                              {item.yajman_name ? (
                                <div style={{
                                  backgroundColor: isPast ? 'var(--color-bg-subtle, rgba(255,255,255,0.04))' : 'rgba(254, 243, 199, 0.12)',
                                  padding: '5px 8px',
                                  borderRadius: 8,
                                  margin: '6px 0',
                                  border: isPast ? '1px solid var(--color-border)' : '1px solid rgba(251, 191, 36, 0.35)',
                                  display: 'flex',
                                  justifyContent: 'space-between',
                                  alignItems: 'center'
                                }}>
                                  <Text style={{ color: isPast ? 'var(--color-text-secondary, #94A3B8)' : '#F59E0B', fontSize: 12, fontWeight: 700 }}>
                                    👑 Yajman: {item.yajman_name}
                                  </Text>
                                  <Tag color={isPast ? 'default' : 'green'} style={{ margin: 0, fontSize: 10 }}>{isPast ? 'CONCLUDED' : 'BOOKED'}</Tag>
                                </div>
                              ) : isPast ? (
                                <div style={{ backgroundColor: 'var(--color-bg-subtle, rgba(255,255,255,0.04))', padding: '4px 8px', borderRadius: 6, margin: '6px 0' }}>
                                  <Text type="secondary" style={{ fontSize: 11 }}>✓ Event Concluded</Text>
                                </div>
                              ) : (
                                <div style={{ backgroundColor: 'var(--color-bg-subtle, rgba(255,255,255,0.04))', padding: '4px 8px', borderRadius: 6, margin: '6px 0', border: '1px dashed var(--color-border)' }}>
                                  <Text type="secondary" style={{ fontSize: 11, fontStyle: 'italic' }}>✨ Open for Sponsorship</Text>
                                </div>
                              )}

                              {item.description && (
                                <Paragraph type="secondary" style={{ fontSize: 12, margin: '4px 0 8px 0', color: 'var(--color-text-secondary, #94A3B8)' }} ellipsis={{ rows: 2 }}>
                                  {item.description}
                                </Paragraph>
                              )}

                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 6, borderTop: '1px solid var(--color-border)' }}>
                                <Text type="secondary" style={{ fontSize: 11, color: 'var(--color-text-secondary, #94A3B8)' }}>
      ...
      >
        ...
        <div style={{ marginBottom: 16, backgroundColor: 'var(--color-bg-layout)', padding: '10px 14px', borderRadius: 10, border: '1px solid var(--color-border)' }}>
          <Text strong style={{ fontSize: 12, color: 'var(--color-text-primary)', display: 'block', marginBottom: 6 }}>
      ...
      >
        <div style={{ textAlign: 'center', padding: '10px 0' }}>
          ...
          <div style={{ display: 'inline-block', padding: 16, borderRadius: 16, backgroundColor: '#FFFFFF', boxShadow: '0 4px 16px rgba(0,0,0,0.15)', border: '1px solid #CBD5E1', marginBottom: 16 }}>
            <QRCode
          ...
          <Text copyable style={{ fontSize: 12, color: 'var(--color-text-primary)', display: 'block', backgroundColor: 'var(--color-bg-layout)', padding: '10px 14px', borderRadius: 10, border: '1px solid var(--color-border)', wordBreak: 'break-all' }}>
