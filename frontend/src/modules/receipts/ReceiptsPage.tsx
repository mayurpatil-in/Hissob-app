      ...
      render: (_: any, record: any) => (
        <Space size={4}>
          ...
          {record.status !== 'cancelled' ? (
            ...
            <Tooltip title="Permanently Purge / Delete from Database">
              <Button
                danger
                type="dashed"
                icon={<DeleteOutlined />}
      ...
      <div className="hissob-stat-row" style={{ marginBottom: 20 }}>
        ...
        <div className="hissob-stat-col">
          <Card className="hissob-stat-card" style={{ borderTop: '4px solid #F97316', borderRadius: 12 }}>
            ...
            <Text type="secondary" style={{ fontSize: 10, whiteSpace: 'nowrap' }}>Issued Documents</Text>
          </Card>
        </div>
      </div>
        ...
        <div style={{ padding: '24px' }}>
          <Form form={form} layout="vertical" onFinish={handleSubmit} initialValues={{ payment_mode: 'cash', receipt_date: dayjs() }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, paddingTop: 12, borderTop: '1px solid #F1F5F9' }}>
              <Button size="large" onClick={() => { setIsModalOpen(false); setEditingReceipt(null); }} style={{ borderRadius: 8, fontWeight: 600 }}>
                Cancel
              </Button>
              <Button
                type="primary"
